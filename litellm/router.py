...
class Router:
    ...
    def _update_kwargs_before_fallbacks(
        ...
        # only falls back when the key is absent, not when its value is None). The guard
        # in async_function_with_retries remains the safety net for paths that bypass this.
        _req_num_retries = kwargs.get("num_retries")
        kwargs["num_retries"] = (
            _req_num_retries if _req_num_retries is not None else self.num_retries
        )
